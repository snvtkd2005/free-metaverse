...
<template>
  ...
  <div
    id="joystick_left_parent"
    class="absolute w-40 h-40 left-0 bottom-16 pointer-events-none"
  ...
  methods: {
    CreateJoytick() {
      ...
      this.Joystick = new Joystick(
        ...
        (axis) => {
          this.UpdateJoystick(axis.axisX, axis.axisY);
        }
      );
    ...
    UpdateJS() {
      // this.$parent.$refs.YJmetaBase.ThreejsHumanChat._YJSceneManager.AddNeedUpdateJS(
      //   this.Joystick
      // );
    },
    SetforcedLandscape(forcedLandscape) {
      this.forcedLandscape = forcedLandscape;
      if (this.Joystick) {
    ...
    // UpdateJoystick(axisX, axisY) {
    //   this.$parent.$refs.YJmetaBase.ThreejsHumanChat.JoystickAxis(axisX, axisY);
    //   if (this.$parent._SceneManager) {
    //     this.$parent._SceneManager.JoystickAxis(axisX, axisY);
    //   }
    //   if (axisX != 0 || axisY != 0) {
    ...
    //     this.times = 0;
    //   }
    //   if (this.display && axisX == 0 && axisY == 0) {
    //     this.times++;
    //     if (this.times > 100) {
    //       this.display = false;
    //     }
    ...
    UpdateJoystick() {
      ...
      if (
        this.display &&
        this.Joystick.axisX == 0 &&
        this.Joystick.axisY == 0
      ) {
        this.times++;
        if (this.times > 100) {
          this.display = false;
