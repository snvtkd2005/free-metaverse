...
<template>
  ...
  <!-- <div class="absolute z-50 w-40 h-40 left-0  bottom-0 pointer-events-none" -->
    ...
    <div id="joystick_left_parent" class=" absolute w-12 h-12 left-2 bottom-14  pointer-events-none"
    :class="display ? ' opacity-100 ' : ' opacity-0 '"

    >

    <div class=" w-12 h-12 flex rounded-full">
      <canvas class="
            self-center
            mx-auto
            rounded-full
            pointer-events-auto
            stop-long-hover
          " ref="joystick" id="joystick_left" width="140" height="140"></canvas>
    </div>
  </div>
  ...
  methods: {
    ...
    CreateJoytick() {
      ...
      this.Joystick = new Joystick(
        ...
        "left",
        document.getElementById("joystick_left"),
        this.$parent.GetPublicUrl() + "img/leftjoystick.png",
        this.$parent.GetPublicUrl() + "img/joystickArea.png"
      );
    ...
    UpdateJoystick() {
      ...
      if (this.Joystick.axisX != 0 || this.Joystick.axisY != 0) {
        ...
        //显示隐藏摇杆
        this.display = true;
        this.$parent.displayJoystick = true;
        ...
        this.laterHiddenJoystick = setTimeout(() => {
          that.display = false;
          that.$parent.displayJoystick = false;
        }, 2000);
      }
      // console.log(" 左摇杆 " + this.display);
    },
