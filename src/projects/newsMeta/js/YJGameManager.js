...
class YJGameManager {
  constructor(_this, parentUI, assetId, avatarData, pos, rota, size, npcManager) {
    ...
    function SetSittingModel(model) {
      ...
      inSitting = true;
      parentUI.Interface.SetInSitting(true);

      return;
      setTimeout(() => {
        //坐到座位上后，全屏播放视频
        document.getElementById("vue3VideoPlayPanel").style.display = "";
        parentUI.$refs.playVideo.play();
      }, 200);
    ...
    this.ClickModel = (hitObject) => {
      ...
      if (modelType == "chair") {
        ...
        // 设置角色坐到椅子上
        SetSittingModel(hitObject.owner);
        //隐藏椅子热点
        _this._YJSceneManager.SetPointObjDisplay("chair002",false);
    ...
    function init() {
      ...
      _this.YJController.AddMovingListener(() => {
        ...
        if(inSitting){
          inSitting = false;
          parentUI.Interface.SetInSitting(false);
    ...
    function CreateThrowObj(parent, modelName) {
      ...
      let mesh = _this._YJSceneManager.checkLoadMesh(_this.GetPublicUrl() + _this._YJSceneManager.GetModelPath(modelName));
