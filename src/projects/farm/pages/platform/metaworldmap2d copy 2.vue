  ...
  <div class="absolute w-full h-full hidden md:flex">
    ...
    >
      ...
      <div class="flex w-full h-auto">
        <div class="w-2/3 h-full bg-gray-300">
          <!-- 搜索 -->
          <div class="flex w-full h-10 gap-5 text-sm leading-10">
            ...
            <div class="flex w-auto h-full gap-1 self-center">
              ...
              <div
                class="bg-gray-100 cursor-pointer rounded-md shadow-md px-2"
                @click="ToChangeOffset()"
              >
                转到坐标
              </div>
            </div>
            ...
            <!-- 回到原点 -->
            <div
              class="w-20 h-full rounded-md shadow-md bg-gray-50"
              @click="ClickSend('回到原点')"
            >
              回到原点
            </div>
          </div>
          ...
          >
            ...
            >
              <!-- 地图 -->
              <div
                v-for="(item, i) in planeDataList"
                :key="i"
                :index="item"
                class="w-32 h-32 flex map-bg"
                :class="
                  selectMapId == item.mapId
              ...
              >
                <div class="w-full h-full">
                  <img
                    class="w-full h-full rounded-none pointer-events-none"
        ...
        <!-- 右侧地块介绍 -->
        <div class="w-1/3 h-full bg-gray-100 relative">
          <!-- 地块介绍 -->
  ...
  mounted() {
    ...
    window.addEventListener("keydown", function (event) {
      ...
      that.playerStyle =
        ...
        //   (that.inputX)  + "px; top:" +
        //  (that.inputY)  + "px; ";
        (that.inputX / 40) * that.oneSize +
        "px; top:" +
        (that.inputY / 40) * that.oneSize +
        "px; ";
      // (that.inputX / that.oneSize) * 40 +
      // "px; top:" +
      // (that.inputY / that.oneSize) * 40 +
      // "px; ";

      // let x = Math.ceil(that.inputX / that.oneSize) * 40;
      // let y = Math.ceil(that.inputY / that.oneSize) * 40;
      // that.ChangeOffset(x, y);
  ...
  methods: {
    ...
    ClickSend(type) {
      ...
      if (type == "回到原点") {
        ...
        this.$parent.$refs.ThreejsHumanChat.SetLocalPlayerToPos({ x: 0, z: 0 });
      }
    },
    ClearJump() {
      this.inJump = true;
    ...
    SetInputXY(x, y) {
      ...
      // 世界坐标转换为地图坐标
      let offsetX = (this.inputX / 40) * this.oneSize;
      let offsetY = (this.inputY / 40) * this.oneSize;
    ...
    CreateCenter(xid, zid, x, z) {
      ...
      planeData.userId = "";
      planeData.img = "mapImg/" + mapId + ".png";
    ...
    Close() {
      ...
      // this.$parent.openMinMap = false;
      this.$parent.openModelPanel = "";
    },
    Update() {
      if (!this.inJump) {
