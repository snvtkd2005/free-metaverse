...
class YJAmmo {
  constructor(scene, enabledAmmo, camera, _this, playerHeight, playerRadius, callback) {
    ...
    function SetGravityActiveFn() {
      ...
      _Global.YJ3D.YJController.SetFlyMountDisplay(!enableGravity && !jumpOnly);
    ...
    function setJumpToFalse() {
      _Global.YJ3D.YJController.setJumpToFalse();
      jumpTime = 0;
      inJumping = false;
    ...
    this.SetMyCtrlRbParent = function (parent, onlyRotaView) {
      ...
      if (onlyRotaView != undefined) {
        _Global.YJ3D.YJController.SetOnlyRotaView(onlyRotaView);
    ...
    function MoveRigidbody(fb, lr, x, y) {
      ...
      if (enableGravity) {
        ...
        //飞行模式
        //飞行模式朝摄像机方向前进后退
        let dirY = _Global.YJ3D.YJController.GetCamTargetWorldDire().y;
    ...
    function YJgenerateObject() {
      ...
      _Global.YJ3D.YJController.SetAmmoPlayer(myCtrlRb, height);
