  ...
  mounted() {
    if (_Global.addEventListener) {
      _Global.addEventListener("选中角色",(targetModel) => {
        if(this.targetModel && this.targetModel != targetModel){
          this.targetModel.removeEvent("设置目标技能进度条");
        }
        this.targetModel = targetModel;
          targetModel.addEventListener(
            "设置目标技能进度条",
          ...
          let targettarget = targetModel.GetTargetModel();
          if(targettarget){
            _Global.applyEvent("选中角色的目标",targettarget);
          }
      });
