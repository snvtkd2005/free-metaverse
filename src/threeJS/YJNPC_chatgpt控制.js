...
class YJNPC {
  constructor(_this, scene, assetId, avatarData, pos, rota, size, npcManager) {
    ...
    function Init() {
      ...
      //     // 查找目标的坐标
      //     // let targetPos = new THREE.Vector3(fromPos.x +1, fromPos.y,fromPos.z);
      //     // let targetPos = _this._YJSceneManager.GetPlayerPos();
      //     // //npc随机在点位之间移动
      //     // navpath = npcManager.GetNavpath(fromPos, targetPos);
      //     // console.error("获取寻路 ", navpath);

      //     // let otherNpcPos = npcManager.GetNpcPos("放牛娃");
      //     // if (otherNpcPos != null) {
      //     //   targetPos.x =  otherNpcPos.x;
      //     //   targetPos.z =  otherNpcPos.z;
      //     //   //npc随机在点位之间移动
      //     //   navpath = npcManager.GetNavpath(fromPos, targetPos);
      //     //   console.log("navpath ",fromPos,otherNpcPos, navpath);
      //     // }

      //     MoveByChooseNavpath(npcManager.GetNpcMovePos("ElwynnForest"));
      //   }
      // }, 3000);
    ...
    function MoveByChooseNavpath(targetPos) {
      let fromPos = playerPosition.clone();
      navpath = npcManager.GetNavpath(fromPos, targetPos);
      // console.log("navpath ", navpath);

    }
    this.SendMsg = function (msg, callback) {
      if(_YJGPT == null){return;}

      if (waitSystemMsg != null) {
        clearTimeout(waitSystemMsg);
      }
      ...
      _YJGPT.SendMsg(msg, (data) => {
        ...
        npcManager.AddChat(npcName,data);
        if (callback) {
    ...
    function ActionToTarget(data) {
      let action = data.action;

      avatar.ChangeAnim(action);
      if(action !="walk" && action != "run" ){
        navpath = [];
        doonce = 0;
      }
      ...
      if (target != "" && target != undefined) {
        ...
        if (action == "walk" || action == "run") {
          ...
          // 查找目标的坐标。先判断是否为角色YJPlayer
          if (target.includes(_this._YJSceneManager.GetNickName())) {
            //npc随机在点位之间移动 
            MoveByChooseNavpath(_this._YJSceneManager.GetPlayerPosReduceHeight());
          } else {
            // 不是用户角色， 再次查找是不是NPC角色
            let otherNpcPos = npcManager.GetNpcPos(target);
            if (otherNpcPos != null) {
              //npc随机在点位之间移动 
              MoveByChooseNavpath(otherNpcPos);
              // console.log("navpath ", target);
            } else {
              // 不是NPC角色，再次查找是不是地名
              // console.log(" 找不到npc,判断是否为地名 ", target);
              otherNpcPos = (npcManager.GetNpcMovePos(target));

              if (otherNpcPos != null) {
                MoveByChooseNavpath(otherNpcPos);
              } else {
                // console.log(" 找不到地名的坐标 ", target);
              }
        ...
        let _YJNPC2 = npcManager.CheckNpc(target);
        if (_YJNPC2 != null) {
          // console.log(" 找到npc ", target);
          let data2 = {};
          data2.from = npcName;
          data2.action = "speak";
          data2.mood = data.mood;
          data2.message = data.message;
          _YJNPC2.SendMsg(JSON.stringify(data2),()=>{
    ...
    let waitSystemMsg = null;
    function SendSystemMsg(s) {
      ...
      data.action = "speak";
      data.message = s;
      _YJGPT.SendMsg(JSON.stringify(data), (text) => {

        // 聊天气泡
        _YJPlayerChat.CreateChatTrans(text.message);
        npcManager.AddChat(npcName,text);
    ...
    function CreateNameTrans(content) {
      ...
      let npcData = npcManager.GetStory(npcName);
      if (npcData != null) {
        if (_YJGPT == null) {
          _YJGPT = new YJGPT(scope, npcData.story, npcName, (data) => {
