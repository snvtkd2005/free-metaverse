<template>
  ...
  <!-- 在线列表 -->
  <div v-if="1 == 0" id="userListPanel" class="
      xl:block
      w-48
      h-48
      absolute
    ...
    <div class="
      ...
      <div v-for="(item, i) in otherUser" :key="i" :index="item.userName" class="relative w-11/12 pl-2 border-b-2 flex"
        :id="item.id">
        <div class="w-2/3 whitespace-nowrap truncate" :class="(item.id == id ? ' pointer-events-none ' : 'cursor-pointer',
        ...
        <div class="absolute right-0 z-auto" :class="(isMainUser && item.id != id
          ? 'pointer-events-auto cursor-pointer '
          : 'pointer-events-none',
          item.audio ? ' opacity-1 ' : ' opacity-0 ')
          " @click.stop="ToggleAudio(item)">
          <div class="w-6 h-6 p-px">
            <img class="w-full h-full" :src="publicUrl + 'images/' + (item.mute ? 'mute' : '') + 'mico.png'
              " alt="" />
          </div>
        </div>
  ...
  <div v-if="1 == 0" class="
    ...
    <div class="w-full h-auto max-h-80 flex flex-col justify-between relative">
      <div ref="roomChateRecode" class="w-72 xl:w-full h-auto max-h-72 overflow-y-scroll overscroll-auto">
        <div v-for="(item, i) in currentChatRecode" :key="i" :index="item.fromId" class="h-auto px-1 text-left break-all"
          :class="id == item.fromId ? ' ' : ' cursor-pointer '" @click="ShowChat(item)">
          <!-- 在大厅中说话 -->
          <div v-if="item.targetUser == ''" class="flex leading-5">
            <div class="whitespace-nowrap w-32 h-5">
  ...
  <div v-if="1 == 0" class="
      ...
      xl:scale-75
    ">
    <div class="absolute left-0 top-0 w-20 h-20" @click="canInputChar = !canInputChar"></div>
  ...
  <div v-if="!connected" class="absolute z-60 w-full h-full top-0 left-0 flex pointer-events-none">
    <div class="w-full flex">
      <div class="
          ...
          inline-block
        ">
        <div class="px-3 self-center mx-auto">正在连接服务器，请稍候。。。</div>
      </div>
    </div>
  </div>
  ...
  <div v-if="meetingInvitation" class="absolute z-60 w-full h-32 top-1/2 left-0 flex">
    <div class="w-96 flex mx-auto rounded-md shadow-md bg-blue-300">
      <div class="
          p-4
          mx-auto
          h-8
        ...
        <div class="mt-2 w-5/6 flex justify-between text-black mx-auto">
          <div class="rounded-md shadow-md p-2 bg-white" @click="meetingHandler('接受')">
            立即接受
          </div>
          <div class="rounded-md shadow-md p-2 bg-white" @click="meetingHandler('拒绝')">
            拒绝
          </div>
        </div>
      </div>
...
import { YJDyncManager } from "/@/threejs/socket/YJDyncManager.js";
...
export default {
  ...
  methods: {
    ...
    // 初始化同步
    InitDync(userData) {
      this.ThreejsHumanChat =
        this.$parent.$refs.YJmetaBase.GetThreejsHumanChat();
      this._YJSceneManager = this.ThreejsHumanChat._YJSceneManager;
      ...
      this._YJDyncManager = new YJDyncManager(this, userData);
    ...
    //添加到服务器后，生成角色
    GeneratePlayer(isLocal, id, platform, nickName, user, num) {
      if (isLocal) {
        this.id = id;
      ...
      if (platform == "pcweb") {
        ...
        let _YJPlayer = new YJPlayer(
          this.ThreejsHumanChat,
          this.ThreejsHumanChat.scene,
          false,
          nickName,
          null,
          (scope) => {
            //在新用户加入后，如果是全员落座、全员挥荧光棒状态，则让新用户镜像执行该动作
            // if (_Global.allSitting != undefined && _Global.allSitting) {
            //   let state = _Global.GetChairPos(this.allPlayer.length - 1);
            //   if (state == null) {
            //     scope.DiplayAll(false);
            //   } else {
            //     scope.ChangeAnim(state.animName);
            //     // console.log(" 在新用户加入后，如果是全员落座 ", state);
            //     scope.SetPlayerPos(state.pos, state.quat);
            //   }
            // }
            if (user.userData != undefined) {
              if (user.userData.animName != undefined) {
                scope.ChangeAnim(user.userData.animName);
              }
            }
          }
        );
        ...
        _YJPlayer.setPlayerDefaultPos(
          this._YJSceneManager.getPlayerDefaultPos()
        );
        ...
        if (user.playerData != undefined) {
          _YJPlayer.NeedChangeSkin();
          _YJPlayer.LoadPlayer(user.playerData.name);
          if (user.playerData.playerState != null) {
            this.$parent.UpdateSkin(
              _YJPlayer,
              user.playerData.name,
              user.playerData.playerState
            );
          } else {
        ...
        _YJPlayer.uid = id;
        _YJPlayer.inChairIndex = -1;
        if (user.userData != undefined) {
          if (user.userData.inChairIndex != undefined) {
            _YJPlayer.inChairIndex = user.userData.inChairIndex;
          }
        }
        ...
        // console.log("添加其他玩家 ", user.userData); 

        _YJPlayer.SetUserData(user.userData);
      }
    ...
    SetPlayerParent(id, parentId) {
      ...
      for (let i = 0; i < this.allPlayer.length; i++) {
        if (this.allPlayer[i].id == id) {
          this.allPlayer[i].player.SetPlayerParent(
            this._YJSceneManager.GetDyncSceneManager().GetModel(parentId)
          );
          return;
        }
      }
    ...
    CreateMapByIdFromServer(sceneModelsDataList) {
      this.ThreejsHumanChat._YJSceneManager.CreateMapByIdFromServer(
        sceneModelsDataList
      );
    },
    ...
    SetAllPlayerPickLightStick(state) {
      if (state.type != undefined && state.type == "单手拿物品") {
        for (let i = 0; i < this.allPlayer.length; i++) {
          // 取消挥荧光棒
          if (state.animName != "idle") {
            state.modelPath = state.modelList[i % 3].modelPath;
          }
          this.allPlayer[i].player.DyncPlayerState(state);
          this.allPlayer[i].player.ChangeAnim(state.animName);
        }
        return;
      }
    },
    DelAllPlayerPickLightStick(state) {
      if (state.type != undefined && state.type == "单手拿物品") {
        for (let i = 0; i < this.allPlayer.length; i++) {
          this.allPlayer[i].player.DyncPlayerState(state);
        }
    ...
    GetAllPlayerSortByUID() {
      let allUID = [];
      for (let i = 0; i < this.otherUser.length; i++) {
        allUID.push(this.otherUser[i].id.length);
      }

      console.error("获取所有用户uid长度", allUID);

      let selfId = this.id.length;
      console.error("自身用户uid长度", selfId);

      let sort = this.otherUser.length;
      if (sort >= 100) {
        sort = 100;
      }
    ...
    GetAllPlayerSortByUID3(chairCount) {
      let allStr = [];
      for (let i = 0; i < this.otherUser.length; i++) {
        allStr.push(this.otherUser[i].id);
      }
      allStr.push(this.id);
      allStr.sort();
      console.log(allStr);
      //提取出已被使用的座位
      let list = this.GetUsedChairIndexList();
      console.log("提取出已被使用的座位 ", list);
      //230520
      let canUseList = [];
      ...
      for (let i = 0; i < chairCount; i++) {
        let has = false;
        for (let j = 0; j < list.length && !has; j++) {
          const jitem = list[j];
          if (jitem == i) {
            has = true;
          }
      ...
      console.log("提取出 未 使用的座位 ", canUseList);
      ...
      for (let i = 0; i < allStr.length; i++) {
        ...
        if (this.id == element) {
          for (let j = 0; j < list.length; j++) {
            const jitem = list[j];
            if (jitem == i) {
              return canUseList[canUseList.length - 1];
            }
          }
          return i;
        }
    ...
    GetAllPlayerSortByUID2() {
      ...
      for (let i = 0; i < this.otherUser.length; i++) {
        ...
        if (isNaN(parseInt(this.otherUser[i].id))) {
          // allUID.push(30000000 + (this.otherUser[i].id).length);
          allStr.push(this.otherUser[i].id);
        } else {
          allUID.push(parseInt(this.otherUser[i].id));
        }
      }
      ...
      if (isNaN(parseInt(this.id))) {
        // selfId = (30000000 + (this.id).length);
        allStr.push(this.id);
      } else {
        selfId = parseInt(this.id);
      }
    ...
    SetAllPlayerState(state) {
      if (state.type != undefined && state.type == "设置角色动作") {
        for (let i = 0; i < this.allPlayer.length; i++) {
          this.allPlayer[i].player.ChangeAnim(state.animName);
        }
        return;
      ...
      if (state.type != undefined && state.type == "全员落座") {
        for (let i = 0; i < this.allPlayer.length; i++) {
          if (i < 100) {
            if (state.quat != undefined) {
              this.allPlayer[i].player.SetPlayerPos(
                state.allChairPos[i],
                state.quat
              );
            }
            ...
            this.allPlayer[i].player.DiplayAll(false);
          }
        }
        return;
      }
    ...
    //230520 获取当前被使用的椅子索引数组
    GetUsedChairIndexList() {
      let list = [];
      for (let i = 0; i < this.allPlayer.length; i++) {
        let index = this.allPlayer[i].player.inChairIndex
        if (index != -1) {
          list.push(index);
        }
      }
    ...
    DyncPlayerState(id, state) {
      ...
      //落座椅子索引 230520
      if (state.type != undefined && state.type == "设置角色入座状态") {
        for (let i = 0; i < this.allPlayer.length; i++) {
          if (this.allPlayer[i].id == id) {
            this.allPlayer[i].player.inChairIndex = state.inChairIndex;
            return;
          }
        }
        return;
      ...
      if (state.type != undefined && state.type == "播放烟花") {
      ...
      if (state.type != undefined && state.type == "切换皮肤") {
        for (let i = 0; i < this.allPlayer.length; i++) {
          if (this.allPlayer[i].id == id) {
            this.allPlayer[i].player.ChangeAvatar(state.avatarName);
            return;
          }
        }
        return;
      }
      if (state.type != undefined && state.type == "换装") {
        for (let i = 0; i < this.allPlayer.length; i++) {
          if (this.allPlayer[i].id == id) {
            this.$parent.ChangeSkin(
              this.allPlayer[i].player,
              null,
              state.playerState
            );
            return;
      ...
      for (let i = 0; i < this.allPlayer.length; i++) {
        if (this.allPlayer[i].id == id) {
          this.$parent._YJGameManager.DyncPlayerState(
            this.allPlayer[i].player,
            state
          );
          return;
    ...
    ClickOtherUser(type, item) {
      ...
      if (type == "传送") {
        this.SetLocalPlayerToOtherUserPos(this.otherUserItem.id);
        this.otherUserItem = null;
        return;
      }
      ...
      if (type == "发送会议邀请") {
        ...
        //向单个用户发送邀请
        this.BoardMsg("receiveMsg", "all", JSON.stringify(fromData));
    ...
    SetUserTRTCstate(useId, video, audio) {
      ...
      for (let i = 0; i < this.otherUser.length; i++) {
        if (this.otherUser[i].id == useId) {
          if (video != undefined) {
            this.otherUser[i].video = video;
          }
          if (audio != undefined) {
            this.otherUser[i].audio = audio;
          }
          return;
        }
      }
