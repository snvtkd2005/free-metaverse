...
class YJAnimatorMMD extends YJAnimator {
  constructor(group, modelPath, owner) {
    super(null);

    console.log(" in YJAnimatorMMD ", group);
    let scope = this;
    // 创建一个时钟对象Clock
    ...
    this.ChangeAnim = function (animName, _animNameFullback) {
      ...
      if (activateAllActions(animName)) {
        ...
        if (scope.transform == undefined) {
          _Global.CreateOrLoadPlayerAnimData().GetExtendAnim(owner.GetAvatarId(), animName, (isLoop, anim) => {
            if (anim != null) {
    ...
    function animToMMDanim(animName, anim) {
      let tracks = [];
    ...
    this.ChangeAnimByAnimData = function (animName, loop, anim) {
      if (animName == "") { return; }
      console.log("添加扩展动画 ", animName, anim);
      anim = animToMMDanim(animName, anim);
      directChangeAllAnim(anim, loop, animName);
      ...
      for (let i = 0; i < modelList.length; i++) {
        ...
        if (element.mesh) {
          ...
          for (let j = 0; j < element.actions.length; j++) {
            const actionD = element.actions[j];
            if (actionD.animName == animName) {
          ...
          if (has != -1) {
            ...
            if (element.currentAction) {
              ChangeAnim3(element.currentAction, mixer, action, loop);
            } else {
              ChangeAnim2(element.currentActionIndex, mixer, action, loop);
            } 
            element.currentAction = currentAction;
          }
        }
      }
      if (has != -1) {
    ...
    function LoadMMD(loader, mmddata, vmddata, callback) {
      ...
      loader.loadWithAnimation(modelFile, vmdFile, function (mmd) {
        ...
        if (!has) {
          vmdIndex = 0;
          modelList.push({ mesh, actions: [], part: { part: part, modelFile }, vmdFiles: [{ vmdFile: vmdFile, vmdIndex: vmdIndex }], currentActionIndex: vmdIndex });
        }
    ...
    function ChangeAnim3(action_old, mixer, action, loop) {
      for (let j = 0; j < mixer._actions.length; j++) {
        let action = mixer.clipAction(mixer._actions[j]._clip);
    ...
    function loadByAnimFile(callback) {
      ...
      _YJLoadAnimation.load(animData.url, (anim) => {
        ...
        anim = animToMMDanim(currentAnimName, anim);
        console.log(anim.tracks);
        animwalk_mmd = anim.tracks;
        // return;

        directChangeAllAnim(anim, animData.loop, currentAnimName);
        return;
    ...
    function directChangeAllAnim(anim, loop, animName) {
      for (let i = 0; i < modelList.length; i++) {
        const element = modelList[i];
        if (element.mesh) {
          let mixer = helper.objects.get(element.mesh).mixer;
          ...
          element.currentActionIndex = element.actions.length;
          element.actions.push({
            action: action,
            targetIndex: element.actions.length, animName: animName, timeScale: 1, weight: 1
            , isLoop: loop
          });
          ChangeAnim2(element.currentActionIndex, mixer, action, loop);
        }
