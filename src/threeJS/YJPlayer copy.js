...
class YJPlayer {
  constructor(_this, scene, local, nickName, controllerCallback) {
    ...
    const Init = () => {
      ...
      _YJPlayerDync = new YJPlayerDync(_this, scene, scope);
    }
    ...
    this.LoadPlayer = function (_playerName) {
      ...
      if (_this.$parent.avatarData.setting.firstPerson == undefined && _this.$parent.avatarData.setting.hasAvatar == undefined) {

      } else {
        if (_this.$parent.avatarData.setting.firstPerson || (!_this.$parent.avatarData.setting.hasAvatar)) {
    ...
    this.SetUserData = function (userData) {
      ...
      // if (userData.animName == "idle" && needAnimName == "walk") {
      // } else {
      //   this.ChangeAnim(userData.animName);
      // }
      // needAnimName = userData.animName;
      this.ChangeAnim(userData.animName);
      ...
      if (oldPos.x != newPos.x || oldPos.z != newPos.z) {
        ...
        // let _pos = playerGroup.position.lerp(newPos, 1);
        // playerGroup.position.set(_pos.x, _pos.y, _pos.z);
