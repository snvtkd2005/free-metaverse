  ...
  constructor(scene, renderer, camera, _this, platform, initCallback) {
    ...
    async function enableHDRFn(b, path, isEnvmap) {
      ...
      if (b && path.indexOf("hdr") > -1) {
        ...
        const [texture] = await Promise.all([
          rgbeLoader.loadAsync(_Global.url.uploadHDRUrl + path),
        ]);
        // environment 
    ...
    async function CreateSkybox(callback) {
      ...
      if (setting.hasEnvmap && envmapPath.indexOf("hdr") > -1) {
        ...
        const [texture] = await Promise.all([
          rgbeLoader.loadAsync(_Global.url.uploadHDRUrl + envmapPath),
        ]);
        // environment
      ...
      if (setting.hasSceneHDR) {
        ...
        const [texture] = await Promise.all([
          rgbeLoader.loadAsync(_Global.url.uploadHDRUrl + setting.envSceneHDRPath),
        ]);
        texture.mapping = THREE.EquirectangularReflectionMapping;
        _Global.hdrEquirect = texture;
        scene.environment = texture;
      ...
      if (hasBackgroundImage) {
        ...
        const [texture] = await Promise.all([
          rgbeLoader.loadAsync(_Global.url.uploadHDRUrl + lightData.backgroundImage),
        ]);
        texture.mapping = THREE.EquirectangularReflectionMapping;
        scene.background = texture;
        scene.environment = texture;
