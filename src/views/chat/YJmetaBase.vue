...
// 在线聊天室 聊天界面 3d形象 聊天
<template>
  <ThreejsHumanChat tabindex="-1" class=" w-full h-full " ref="ThreejsHumanChat"
    id="ThreejsHumanChat" />
</template>
...
export default {
  ...
  created() {
    this.sceneData = this.$parent.avatarData;
  ...
  methods: {
    ...
    UpdateCheckWindowResize() {
      ...
      if (this.$parent.$refs.container) {

        if (
          this.windowWidth == this.$parent.$refs.container.clientWidth &&
          this.windowHeight == this.$parent.$refs.container.clientHeight
        ) {
        } else {
          this.windowWidth = this.$parent.$refs.container.clientWidth;
          this.windowHeight = this.$parent.$refs.container.clientHeight;
    ...
    GetAvatarData(playerName) {
      ...
      if (this.$parent.playerAnimData) {
        let sceneData = this.$parent.playerAnimData.sceneData;
        for (let i = 0; i < sceneData.length; i++) {
          if (sceneData[i].name == playerName) {
            return sceneData[i];
          }
        }
        return null;
      }
      for (let i = 0; i < this.sceneData.sceneData.length; i++) {
        if (this.sceneData.sceneData[i].name == playerName) {
          return this.sceneData.sceneData[i];
        }
      }
      return null;
    },
