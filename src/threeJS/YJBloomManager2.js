...
class YJBloomManager2 {
  constructor(_this, scene, camera, renderer,bloomData) {
    ...
    const toneMappingOptions = {
      ...
      Reinhard: THREE.ReinhardToneMapping,  // exposure 1.4
      Cineon: THREE.CineonToneMapping,  // exposure 1.2
    ...
    function Init() {
      ...
      if(hasGUI){
        ...
        params.bloomTexStrength = bloomData.bloomTexStrength;
      }
      ...
      uniform float baseTexStrength;
      uniform float bloomTexStrength;
      uniform sampler2D baseTexture;
			...
			void main() {
				...
				gl_FragColor = ( vec4(baseTexStrength ) * texture2D( baseTexture, vUv ) + vec4(bloomTexStrength ) * texture2D( bloomTexture, vUv ) );
				// gl_FragColor = ( texture2D( baseTexture, vUv ) + vec4( 0.1 ) * texture2D( bloomTexture, vUv ) );
				// gl_FragColor = ( texture2D( baseTexture, vUv ) );

			}`;

      uniforms= {
        baseTexStrength: { type: "f", value: params.baseTexStrength },
        baseTexture: { value: null },
        bloomTexture: { value: bloomComposer.renderTarget2.texture },
        bloomTexStrength: { type: "f", value: params.bloomTexStrength },
      };
      ...
      if (hasGUI) {
        ...
        toneMappingfolder.add(params, 'toneMapping', Object.keys(toneMappingOptions)).onChange(function (value) {
          renderer.toneMapping = toneMappingOptions[params.toneMapping];
          render();
        });
        ...
        folder.add(params, 'baseTexStrength', 0.1, 4).onChange(function (value) {
          uniforms.baseTexStrength.value = Number(value);
        });
        folder.add(params, 'bloomTexStrength', 0.1, 2).onChange(function (value) {
          uniforms.bloomTexStrength.value = Number(value);
        });
