...
<template>
  ...
  <!-- 角色换装选择  -->
  <div class="  absolute z-30 right-0 top-0 w-1/2 xl:w-1/2 h-full overflow-hidden pointer-events-none  ">
    <SkinPanel class=" w-full h-full " ref="SkinPanel" :skinData="skinData"></SkinPanel>
  </div>

  <!-- 角色选择 和 场景选择 -->
  <div class="absolute top-10 xl:top-10 left-10 xl:left-10 z-60 w-64 xl:w-auto h-1/2  ">
    <!-- 角色选择 -->
    <div class="
                                xl:w-auto
                                w-auto
    ...
    <div class=" 
      ...
      <div class="
        ...
        <div v-for="(item, i) in sceneImgPath" :key="i" :index="item.img" class="
          ...
          <div class=" self-center mx-auto">
            <img class=" w-full h-full    object-fill" :src="publicUrl + item.icon" />
          </div>

          <div v-if="selectSceneName == item.name" class=" absolute -bottom-2 -right-2 ">
            <img class="  w-4 h-4 xl:w-full xl:h-full object-fill" :src="publicUrl + 'images/spUI/select.png'" />
          </div>
        </div>
      </div>
