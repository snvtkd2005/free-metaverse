...
class YJPlayerFireCtrl {
	constructor(YJController, _YJPlayer) {
		let scope = this;
		...
		this.updateEquip = function () {
			YJController.directUpate("equip");
		}
		...
		function hyperplasia(modelData, num, count, times) {
			...
			let pos = YJController.GetPlayerWorldPos2();
		...
		function GetShootingStartPosFn() {
			let pos = YJController.GetPlayerWorldPos();
			let weaponModel = _YJEquip.getWeaponModel();
			if (weaponModel) {
		...
		this.shootTargetPos = function (tagetPos, time, callback) {
			...
			temp.position.copy(YJController.GetPlayerWorldPos());
			temp.lookAt(tagetPos.clone());
			temp.translateZ(vaildAttackDis);
			tagetPos.copy(temp.position);
			//有效攻击
			_Global.DyncManager.shootTarget(YJController.GetPlayerWorldPos(), tagetPos, time, "npc", fireParticleId, (target) => {
				if (target) {
		...
		this.GetWorldPos = function () {
			return YJController.GetPlayerWorldPos2();
		}
		this.GetWorldPosHalfHeight = function () {
			return YJController.GetPlayerWorldPos();
		}
		this.GetDamageTextPos = function () {
			return YJController.GetPlayerWorldPos3();
		}
		...
		function CheckCanAttack() {
			...
			let playerPos = YJController.GetPlayerWorldPos();
			playerPos.y += 1;
		...
		function CheckColliderBetween(fromPos, targetPos) {
			...
			// console.log(_Global.YJ3D._YJSceneManager.GetAllColliderAndLand(), hits);
			if (hits.length > 0) {
		...
		this.SetPlayerState = function (e, _animName) {
			...
			switch (e) {
				...
				case "death":
					...
					//角色不可控制、显示倒计时
					YJController.SetCanMoving(false);
					YJController.CancelMoving();
		...
		function Init() {
			...
			if (baseData == null) {
				...
				baseData = YJController.GetUserData().baseData;
