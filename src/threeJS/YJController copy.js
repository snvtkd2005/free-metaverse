...
class YJController {
  constructor(scene, camera, domElement, _this) {
    ...
    var PLAYERSTATE = { NORMAL: -1, DEAD: 0, ATTACK: 1, INTERACTIVE: 2, SITTING: 3 };
    ...
    // 鸟瞰时，摄像机位置
    let niaokanCamPos = new THREE.Vector3(0, 100, 0);
    // 鸟瞰时，摄像机lookat位置
    ...
    function directToNiaokan() {
      ...
      if (_this.$parent.avatarData && !_this.$parent.avatarData.setting.hasCamRaycast) {
        return;
      }
    ...
    function ChangeCtrl() {
      ...
      hasCamRaycast = _this.$parent.avatarData && _this.$parent.avatarData.setting.hasCamRaycast;
      //
      viewStateEnum = "人视";
      ...
      if (contrlState == CONTRLSTATE.KEYBOARD_MOUSE) {
        ...
        // 第一人称视角
        if (_this.$parent.avatarData && _this.$parent.avatarData.setting.firstPerson) {
          wheelCurrentValue = 0;
          wheelMin = 0;
          wheelMax = 0;
          currentWheelValue = 0;
        ...
        // 第一人称视角
        if (_this.$parent.avatarData.setting.firstPerson) {
          camera.position.set(0, 0, 0);
          Display(false);
          return;
        }
    ...
    let display = true;
    function Display(b) {
      if (display == b || _player == null) { return; }
      if (b) {
        _YJPlayer.GetPlayerGroup().scale.set(1, 1, 1);

        if (viewState == 1) { viewState = 0; }
      } else {
        _YJPlayer.GetPlayerGroup().scale.set(0, 0, 0);
      }
      display = b;
    }
    ...
    this.onMouseWheel = function (event) {
      ...
      // console.log("wheelCurrentValue = "+ wheelCurrentValue);
      //当离摄像机最近的时候，隐藏角色
      Display((wheelCurrentValue != wheelMax));
    }
    ...
    this.onTouchMove = function (event) {
      if (_this.$parent.avatarData && _this.$parent.avatarData.setting.rightJoystick) {
        return;
      }
    ...
    this.onKeyDown = function (event) {
      ...
      switch (event.code) {
        ...
        case 'Space':
          ...
          event.preventDefault();
          event.stopPropagation();
          if (_this.$parent.avatarData.setting.keySpace != undefined && !_this.$parent.avatarData.setting.keySpace) { return; }
          this.ClickJump();

          if (!_this.$parent.avatarData.setting.canEnableGravity) { return; }
          moveUp = true;
          break;
    ...
    this.onKeyUp = function (event) {
      ...
      switch (event.code) {
        ...
        case 'KeyM':
          //  开关地图
          if (_this.$parent.clickOpenPanel) {
            _this.$parent.clickOpenPanel("小地图");
          }
          break;
        case 'KeyC':
          if (!_this.$parent.avatarData.setting.keyC) { return; }
          //  切换控制模式
          ChangeCtrlStateCall();
          break;
        case 'KeyG':
          ...
          break;
        case 'KeyQ':
          if (!_this.$parent.avatarData.setting.keyQ) { return; }
          //视角切换 
          ChangeCameraFarFn();
          break;
    ...
    this.ResetPlayerPosRota = function () {
      // if (ammoHeight) { 
      //   playerParent.position.set(0, -ammoHeight / 2, 0);
      //   playerParent.rotation.set(0, -3.14 / 2, 0);
      // }
      // //角色初始角度
      // _YJAmmoPlayer.rotation.y = 3.14;
    ...
    */
    this.SetChangeViewBegin = function () {
      ...
      if (!_this.$parent.avatarData.setting.hasAerialView) {
        // 不需要鸟瞰时，把摄像机放到角色后面
        ChangeCtrl();
        return;
    ...
    */
    ...
    //设置相机坐标和相机lookat坐标，
    this.SetCamPosAndRota = (data, callback) => {
      ...
      // let length = 1;
      // movingTween = new TWEEN.Tween(lep_onlyMoveCamera).to(length, 3000).easing(TWEEN.Easing.Cubic.InOut)
      // let updateTargetPos = () => {
      //   currentCamPos.lerp(onlyMove_camPos, lep_onlyMoveCamera);
      //   camera.position.set(currentCamPos.x, currentCamPos.y, currentCamPos.z);
      //   camera.quaternion.rotateTowards(onlyMove_camRota, lep_onlyMoveCamera / 10);
      // }
      // movingTween.onUpdate(updateTargetPos);
      // movingTween.start() // 启动动画
      // movingTween.onComplete(() => {
    ...
    function GetInvaildCastObj(intersects_collider) {
      let camRaycastMode = _this.$parent.avatarData.setting.camRaycastMode;


      if (camRaycastMode == "near" || camRaycastMode == undefined) {
    ...
    this.updateSend = function () {
      ...
      if (oldrotateY == newrotateY && oldAnimName == animName && oldAnimWeight == animWeight
        && oldPos.distanceTo(newPos) < 0.01) {
        // console.log("needCloseDonce = " + needCloseDonce);
        if (needCloseDonce >= 360000) {
          // 判断10分钟无操作，断开连接
          // _this.$parent.SelfCloseConnect();
          // return;
        }
        needCloseDonce++;
      ...
      //掉入地下，刷新玩家到地面上重新开始
      // if (newPos.y < -5) {
      //   _this._YJSceneManager.SetPlayerPos({ x: 0, y: 10, z: 0 });
      //   return;
      // }
    ...
    this.update = function () {
      ...
      if (moveForward || moveLeft || moveRight || moveBackward) {
        ...
        _this._YJSceneManager.UpdateLightPos();
