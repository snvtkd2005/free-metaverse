...
class YJSkillParticleManager {
  constructor() {
    ...
    function shootTargetFn2(startPos, target,speed, callback) {
      ...
      group.position.copy(startPos);
      let trailRenderer = { group: group, trail: new YJTrailRenderer( _Global.YJ3D.scene, group) };

      trailRenderer.trail.start();
      _Global.YJ3D._YJSceneManager.AddNeedUpdateJS(trailRenderer.trail);
    ...
    this.shootTargetFn = async function (startPos, target, speed, particleId, callback) {
      ...
      let path =  _Global.url.uploadGroupUrl + particleId + "/" + "data.txt" + "?time=" + new Date().getTime();
