...
<template>
  <!--  -->
  <!-- 场景切换loading背景图 -->
  <div v-if="loading" class=" loading  pointer-events-none absolute z-50 left-0 top-0 w-full h-full">
    <img class=" loadingImg w-full h-full" :src="path + sceneImg" alt="">
    <div class=" loadingProcessText absolute left-1/2 transform -translate-x-28 w-56 h-5 bottom-12 border  ">
      <div>{{ "正在进入场景，" + loadingProcess + '%...' }}</div>
    </div>
    <div class=" loadingProcess absolute left-1/2 transform -translate-x-28 w-56 h-5 bottom-10 border  ">
      <div class=" loadingProcessChild h-full bg-green-500  " :style="'width: ' + loadingProcess + '%'"></div>
    </div>
  </div>
</template>
