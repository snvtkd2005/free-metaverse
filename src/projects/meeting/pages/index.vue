...
export default {
  ...
  mounted() {
    ...
    this.Interface = new Interface(this, 2);
    ...
    if (!localStorage.getItem("playerState")) { 
      localStorage.setItem("playerState", "male_0_0_0_0_0_0");
    } else {
      let sp = localStorage.getItem("playerState").split("_");
      console.log(sp.length,localStorage.getItem("playerState"));
      if (sp.length != 7) {
        localStorage.setItem("playerState", "male_0_0_0_0_0_0");
      }
    }
  ...
  methods: {
    ...
    UpdateSkin(_YJPlayer, playerName, playerState) {
      ...
      let gander = sp[0];
      // if (gander == undefined || gander == null) { gander = "male"; }
      // 从后台读取换装数据  
      if (gander == "male") {
        skinData = _Global.GetTexiaoData.maleSkinData;
      }
      if (gander == "female") {
        skinData = _Global.GetTexiaoData.femaleSkinData;
      }

      // 从后台读取换装数据
      console.error(" skinData ", skinData);


      for (let i = 0; i < skinData.length; i++) {
        skinData[i].selected = parseInt(sp[i+1]);
      }

      if(skinData[5].selected != -1){
        skinData[2].selected = -1;
        skinData[3].selected = -1;
      }
      ...
      for (let i = 0; i < skinData.length; i++) {
        ...
        if (
          ...
          if(element.selected == -1){continue;}
          targetPath = element.modelPath[element.selected];
          _YJPlayer.ChangeSkin(
            targetPath,
            element.part,
