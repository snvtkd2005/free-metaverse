...
<template>
  <div class="absolute top-0 left-0 w-full h-full bg-gray-400">
    ...
    <!-- 角色选择 和 场景选择 -->
    <div
      class="absolute top-2 xl:top-10 left-2 xl:left-32 z-60 w-64 xl:w-auto h-1/2"
    >
      <!-- 角色皮肤选择 -->
      <div class="xl:w-auto w-auto h-auto self-center mx-auto">
        <!-- 列表 -->
        <div
          class="flex-row w-full max-w-2xl h-auto gap-3 xl:gap-0 xl:h-2/3 justify-items-start text-white"
        >
          <div
            v-for="(item, i) in skinData"
            :key="i"
            :index="item.img"
            class="w-auto h-20 xl:w-auto xl:h-auto self-center mx-auto flex-row relative mb-3 xl:mb-10"
          >
            ...
            <div class="mt-px xl:mt-4 flex gap-5">
              <div
                v-for="(item2, i2) in item.icon"
                :key="i2"
                class="w-16 h-16 xl:w-32 xl:h-32 transform hover:scale-110 self-center mx-auto cursor-pointer flex relative rounded-full"
