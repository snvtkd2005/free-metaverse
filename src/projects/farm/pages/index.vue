...
<template>
  <div :class="this.avatarData.setting.onlyLandscape ? 'main' : ''">
    ...
    <div class=" hidden absolute top-0 left-0 cutimg overflow-hidden">
      <canvas id="id-wall-canvas" class="bg-white"> </canvas>
    </div>
    <!-- 视角远近滑块 -->
    ...
    <!-- 角色头像 -->
    <gameUI v-if="hasGameUI" class="absolute z-0 left-0 top-0" ref="gameUI" />
  ...
  mounted() {
    ...
    document.addEventListener("visibilitychange", () => {
      if (document.hidden) {
        ...
        //  _Global.userTouched = false;
        // 回来的时候打开声音
        
        if (_Global.SetVideoMuted) {
          _Global.SetVideoMuted(true);
        } 
      } else {
        _Global.inFocus = false;
        if (_Global.CallLoadSQL) {
          _Global.CallLoadSQL();
        }
        if (_Global.SetVideoMuted) {
