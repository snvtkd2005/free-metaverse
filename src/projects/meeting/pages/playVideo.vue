...
<template>
  ...
  <!--  -->
  <div
    id="vue3VideoPlayPanel"
    ref="vue3VideoPlayPanel"
    class="vue3VideoPlayPanel"
  >
    <vue3VideoPlay
    :id="videoId"
      class=""
...
export default {
  ...
  data() {
    return {
      video_url: "",
      type: "video/mp4",
      // video_url: " http://play-qukan.cztv.com/live/1681283679901088.m3u8",
      // type: "m3u8",
      //视频
      options: {
        width: "800px", //播放器高度
  ...
  methods: {
    ...
    PlayVideoStream(url, isMuted) {
      ...
      console.error("加载视频 ", url);

      if (isMuted != undefined) {
        this._video.muted = isMuted;
        this.$refs.videoPlayer.muted = isMuted;
        setTimeout(() => {
          this.SetVideoMuted(isMuted);
        }, 1000);
      }
