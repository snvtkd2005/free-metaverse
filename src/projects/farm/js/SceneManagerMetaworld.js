...
class SceneManager {
  constructor(scene, renderer, camera, _this, modelParent, indexVue, callback) {
    ...
    this.AddAmmoPlane = function (id) {
      ...
      return;
      let folderBase = GetFolderBaseByMapid(id);
      if (folderBase == null) {
        return;
      }
      let path = _this.$uploadSceneUrl + folderBase + "/" + folderBase + "_scene.txt"
      _this._YJSceneManager.LoadAssset(path, (data) => {
        let modelList = [];
        console.log(" 获取地图中模型数据 ", id, data);
        for (let i = 0; i < data.length; i++) {
          const element = data[i];
          modelList.push(element);
        }
        _this._YJSceneManager.CreateSceneByMapid(id, modelList);
      });
    }
    ...
    function LoadSceneModelListByFolderBase(mapData) {
      ...
      let path = _this.$uploadSceneUrl + mapData.folderBase + "/" + mapData.folderBase + "_scene.txt"
      _this._YJSceneManager.LoadAssset(path, (data) => {
        let modelList = [];
        console.log(" 获取地图中模型数据 ", mapData.id, data);
        for (let i = 0; i < data.length; i++) {
          const element = data[i];
          modelList.push(element);
        }
    ...
    this.CallLoadSceneDataByFolderBase = (folderBase,callback) => {
      ...
      let path = _this.$uploadSceneUrl + folderBase + "/" + folderBase + "_setting.txt" + "?" + new Date().getTime()
      _this._YJSceneManager.LoadAssset(path, (data) => {
