...
class YJDragModel {
  constructor(_this, scene, camera, renderer, domElement,StartDragCallback,EndDragCallback) {
    ...
    function init_DragModel() {
      ...
      dragControls.addEventListener('dragstart', function (event) {
        ...
        StartDrag();
        _this.YJController.enabled = false;

      });
      dragControls.addEventListener('dragend', function (event) {
        // console.log(" 结束  拖拽模型  ");
        EndDrag();
        _this.YJController.enabled = true;
      });
    }
    ...
    function StartDrag() {
      if(StartDragCallback){StartDragCallback();}
      
    }
    function EndDrag() {
      if(EndDragCallback){EndDragCallback(drag_objects[0]);}
      drag_objects.splice(0, drag_objects.length);//清空数组
      ...
      if (dis < 0.01) {
        _this.YJSceneModelCtrl.popStep();
        drag_objects[0].position.set(dragStartPosX, dragStartPosY, dragStartPosZ);
        return;
      } else {
        _this.YJSceneModelCtrl.updateObjPos(drag_objects[0], dragEndPos);
      }
