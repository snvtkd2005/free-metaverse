...
// 挖掘机
class YJCar {
  constructor( scene, id, carData, pos, rotaV3, size, _state, callback, owner) {
    let scope = this;
    this.id = id;
    this.pos = null;
    this.rota = null; 
    ...
    function Init() {
      ...
      if (carData.wheelPath != "") {
        ...
        _YJLoadModel.load(id, _Global.url.uploadUrl + modelPath,
          pos, new THREE.Vector3(0, Math.PI / 2, 0), size, false, null, (scope) => {
            ...
            _YJLoadModel.load(id, _Global.url.uploadUrl + modelPath, pos, new THREE.Vector3(0, Math.PI / 2, 0), size, false, null, (scope) => {
              let model = scope.GetModel();
              model.visible = false;
    ...
    this.updateCarData = function (_carData) {
      ...
      carData = _carData;
      _Global.YJ3D._YJSceneManager.RemoveCollider(chassisMesh);
      _Global.YJ3D._YJSceneManager.ClearMesh(chassisMesh);
      carBodyObj = _carBodyObj.clone();

      chassisMesh = null;
      for (let index = wheelMeshes.length - 1; index >= 0; index--) {
        _Global.YJ3D._YJSceneManager.RemoveCollider(wheelMeshes[index]);
