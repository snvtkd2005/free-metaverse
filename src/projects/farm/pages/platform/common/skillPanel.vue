...
<template>
  ...
  <div v-if="actionBar" class=" relative flex w-full pointer-events-none  h-12 text-sm text-white">
    ...
    <div v-if="hover" class=" absolute  w-2 h-2  text-white  " :style="' position:absolute; left:' +
      ...
      ">
      <div
        class=" absolute bottom-0 left-0   text-center origin-bottom transform -translate-y-6 -translate-x-10 w-20 h-auto bg-black bg-opacity-40 ">
  ...
  <div v-if="actionBar" class="  flex  absolute right-0 bottom-0 w-40 pointer-events-auto  h-40 text-sm text-white">
    ...
    <div class=" absolute left-0 top-0 w-16 h-16 flex bg-black bg-opacity-50 rounded-full " @click="clickSkill('攻击')">
