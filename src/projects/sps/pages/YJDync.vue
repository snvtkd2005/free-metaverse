<template>
  ...
  <div id="userListPanel"
    ...
    <div class=" mt-10 text-left h-auto max-h-96 overflow-y-auto overscroll-auto text-white">
      <div v-for="(item, i) in otherUser" :key="i" :index="item.userName" class=" relative w-11/12  pl-2 flex "
        :id="item.id">
  ...
  <!-- 聊天记录 -->
  <div class=" absolute right-0 bottom-0 origin-bottom-right pointer-events-none "
    :class="isMobile ? ' transform scale-50 ' : '  '">
    <img class="  origin-bottom-right transform scale-50  " :src="publicUrl + 'chatBG.png'" alt="">
    <div>
      <div class="
                      xl:flex
                      xl:w-96
        ...
        <div class="w-full h-auto max-h-48 flex flex-col justify-between relative">

          <div ref="roomChateRecode" class=" w-72  xl:w-full h-auto max-h-72 overflow-y-auto overscroll-auto">
            <div v-for="(item, i) in currentChatRecode" :key="i" :index="item.fromId"
              class="h-auto px-1 text-left break-all " :class="
                id == item.fromId
                  ? ' '
                  : ' cursor-pointer '
              " @click="ShowChat(item)">
              ...
              <div v-if="item.targetUser == ''" class="flex leading-5">
                <div class=" whitespace-nowrap w-auto  h-5 ">[{{ item.fromUser }}]：</div>
      ...
      <!-- 输入区域 -->
      <div class=" absolute  bottom-10 chatinput
         h-10 flex bg-gray-300 bg-opacity-50 text-white rounded-lg pointer-events-auto">
        <!-- 悄悄话 私聊提示 -->
        <div class=" absolute left-0 -top-6 w-full h-10 ">
        ...
        <div class="w-full h-10 ">
          <input ref="roomInput" class="
                        w-72 xl:w-64 px-2
                              ...
                              h-full 
                              resize-none
                            " type="text" placeholder="" v-model="currentChatStr" @focus="removeThreeJSfocus"
