...
class YJuserData {
  constructor(indexVue) {
    ...
    let avatarId = "";
    let userName = "aa"; 
    this.GetPlayerAnimData = function() {
    ...
    this.ClickSelectPlayerOK = function(selectPlayerName, userName){
      ...
      indexVue.$refs.YJmetaBase.ClickSelectPlayerOK(userData);
      _Global.YJ3D.SetNickName(userName);
    }
    this.Enter = function(folderBase) {
      // console.log(" 进入 enter ",folderBase);
      // 用户首次进入先使用默认角色
      avatarId = PlayerAnimData.defaultUser.avatarId;
      ...
      // 如果场景限制角色，则使用场景限制角色
      if (sceneData.avatarList && sceneData.avatarList.length > 0) {
        avatarId = sceneData.avatarList[indexVue.Utils.RandomInt(0, sceneData.avatarList.length - 1)].folderBase;
      }
