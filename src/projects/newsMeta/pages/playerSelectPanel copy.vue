...
<template>
  ...
  <!-- 角色选择 和 场景选择 -->
  <div class="absolute top-2 md:top-20 left-10 md:left-10 z-60 w-auto md:w-1/2 h-1/2  ">
    <!-- 角色选择 -->
    <div class="
        md:w-auto
        w-auto
      ...
      ">
      <div class=" flex mb-5 ">
        <div class=" text-left self-center  h-10 leading-10 text-2xl ">
      ...
      <div class="
        ...
        md:w-24 md:h-24
          ...
          <div class="  w-full h-full self-center mx-auto">
            <img class="  w-full h-full rounded-full object-fill" :src="publicUrl + item.img" />
          </div>
          
          <div v-if="selectPlayerName == item.name" class=" absolute bottom-0 right-0 ">
            <img class=" w-4 h-4 xl:w-6 xl:h-6 object-fill" :src="publicUrl + 'images/spUI/select.png'" />
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- 场景选择 -->
    <div class=" hidden absolute -right-60 top-0 
    md:static 
      md:mt-10
        w-auto
        h-auto  
  ...
  <!-- 右侧角色模型展示 -->
  <div class="  absolute z-50 right-0 top-0 w-1/2 md:w-1/2 h-full overflow-hidden   ">
    <div id="contain" class="w-full h-full  " ref="YJ3dscene"></div>

    <div class=" absolute left-0 bottom-2 md:bottom-20 w-full  md:w-1/2 transform md:translate-x-1/2 ">

      <!-- 输入昵称 -->
      <div class=" w-full flex">
        <div class="flex w-auto h-7 px-2 mx-auto">
...
export default {
  ...
  mounted() { 
    ...
    if (_Global.userData == undefined || _Global.userData == null) {
      ...
      this.userName = _Global.userData.nickname;
