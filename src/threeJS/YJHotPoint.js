...
class YJHotPoint {
  constructor(_this, scene, id, modelName, modelPath, pos, rota, size, callback) {
    ...
    this.GetPointObjWorldDirection = function () {
      if (pointObj) { 
        return pointObj.getWorldDirection(new THREE.Vector3());
      }
    ...
    this.SetPointBlink = function () {
      ...
      let interval = setInterval(() => {
        ...
        pointObj.material.opacity = opacity;
        if (opacity <= 0) {
          direction = 1;  
        }
        if (opacity >= 1) {
    ...
    function Init() {
      ...
      let _YJLoadModel = new YJLoadModel( group);
      _YJLoadModel.load(modelName, modelPath, pos, rotaV3, size, true, null, () => {
        setTimeout(() => {
            ...
            if (item instanceof THREE.Mesh) {
              ...
              if (item.name.indexOf("trigger") > -1) {
                ...
                if (item.name.indexOf("point") > -1) {
                  ...
                  if (hotPointData) {
                    let modelData = { id: hotPointData.id, type: hotPointData.pointType };
                    pointObj.modelData = modelData;
                  }
                  pointObj.owner = scope;
                  return;
                }
    ...
    function CreatePlane() {
      let hotPointData = _this.$parent.sceneData.hotPointData;
      let size = hotPointData.size;
    ...
    function CreateHotPoint(parent, pos, size) {
      ...
      let hotPointSetting = _this.$parent.sceneData.hotPointData;
    ...
    function CreateVideo() {
      ...
      // console.log("创建视频 ",type);
      _YJVideo = new YJVideo( hotPointData.id, pointObj, type);
    }

    this.GetVideoJS = function(){
      return _YJVideo;
    }
    ...
    this.SetOverlapPlayer = function (player) {
      ...
      if (hotPointData) {
        ...
        if (hotPointData.pointType.includes("进入区域才显示")) {
          if (pointObj) {
            pointObj.visible = inArea;
          }
        ...
        if (hotPointData.pointType.includes("进入区域才可点击")) {
          if (inArea && hitArea) {
            _Global.YJ3D._YJSceneManager.AddCanHitModel(hitArea);
          } else {
            _Global.YJ3D._YJSceneManager.RemoveCanHitModel(hitArea);
    ...
    function updateState() {
      ...
      // console.log(hotPointData);
      if (hotPointData.pointType.includes("播放视频a使用id")) {
        if (pointObj) {
          pointObj.visible = true;
          CreateVideo();
        }
      ...
      if (hotPointData.pointType.indexOf("热点") > -1) {
        ...
        //   pointObj.visible = true;
        // }
        // pointObj.position.set(pos.x, pos.y, -pos.z);
        // pointObj.visible = false;

        CreateHotPoint(pointObj, new Vector3(0, 0, 0), pointObj.scale);
      }
      ...
      if (hotPointData.pointType.indexOf("代码控制显示") > -1) {
        ...
        if (hotPointData.id.indexOf("text_") > -1) {
          ...
          if (pointObj) {
            ...
            let hotpointImgUrl2 = _this.$parent.sceneData.hotPointData.hotpointImgUrl2;
            if (hotpointImgUrl2) {
            ...
            let type2 = _this.$parent.sceneData.hotPointData.type2;
            if (type2 && type2 == "anim") {
