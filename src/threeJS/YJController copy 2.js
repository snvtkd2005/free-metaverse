...
class YJController {
  constructor(scene, camera, domElement, _this) {
    ...
    // 旋转视角时是否使用阻尼
    let canDampingRota = true;
    let b_LerpRotaBase_leftMouse = false;
    let lerpX = 0;
    let lerpY = 0;
    let lerpX_d = 1;
    let lerpY_d = 1;
    ...
    let dampingTargetV3 = new THREE.Vector2(0, 0);
    // 滑动阻尼 
    function LerpRotaBase_leftMouse() {

      dampingCurrentV3.x = lerpX * rotaDirection * lerpXY_scale;
      dampingCurrentV3.y = lerpY * rotaDirection * lerpXY_scale;

      if (movingTween != null) {
        movingTween.stop();
      }
      movingTween = new TWEEN.Tween(dampingCurrentV3).to(dampingTargetV3, 1500).easing(TWEEN.Easing.Cubic.InOut)
      let updateTargetPos = () => {
    ...
    */
    ...
    this.updateSend = function () {
      ...
      userData.mountName = _YJPlayer.GetMountName();

      userData.dyncDisplay = dyncDisplay;
    ...
    let oldAmmoPlayerPos = null;
    let camBaseParentOffsetPos = new THREE.Vector3(0,0,0);
    let camBaseParentDefaultPos = new THREE.Vector3(0,0,0);
    this.update = function () {
      ...
      // let pos = _YJAmmoPlayer.getWorldPosition(new THREE.Vector3());
      let pos = _YJAmmoPlayer.position;
      if(oldAmmoPlayerPos == null){
        oldAmmoPlayerPos = pos.clone();
      }

      // if(camBaseParent.parent == _YJAmmoPlayer){
      //   camBaseParent.parent = scene;
      // }


      camBaseParentOffsetPos.y = (pos.y-oldAmmoPlayerPos.y)*-1;
      camBaseParentOffsetPos.lerp(camBaseParentDefaultPos,0.01);

      // camBaseParentOffsetPos.lerp(pos,0.01);


      camBaseParent.position.copy(camBaseParentOffsetPos.clone());
      // camBaseParent.position
      oldAmmoPlayerPos = pos.clone();
