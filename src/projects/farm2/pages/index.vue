...
<template>
  ...
  <!-- 加载loading -->
  <loadingPanel class="absolute z-40 left-0 top-0" ref="loadingPanel" />
  ...
  <div v-if="isMobile && contrlState == 0">
    <JoystickLeftPanel class="absolute z-20" ref="JoystickLeftPanel" />
    <!-- <JoystickRightPanel class="  " ref="JoystickRightPanel" /> -->
  </div>

  <farmInsetPanel ref="insetPanel" v-if="isInsertPanel" />

  <farmloading class="absolute z-50 left-0 top-0" />
</template>

<script>
// 三维页
import ThreejsHumanChat from "/@/threeJS/threeSHJKGChat.vue";
import farmloading from "./Loading.vue";
...
// 加载进度页
import loadingPanel from "./loadingPanel_farm.vue";
...
import farmInsetPanel from "./farmInsetPanel.vue";
...
export default {
  ...
  mounted() {
    ...
    this.avatarData.isMobile = this.isMobile;
