...
<template>
  <div :class="this.avatarData.setting.onlyLandscape ? 'main' : ''">
    ...
    <div
      ...
      class="absolute left-1/2 bottom-0 w-96 h-48 transform -translate-x-48 scale-75"
      @click="ClickNiaokan()"
    >
      <map2d ref="map2d" :inEditor="avatarData.setting.inMinMapEditor" />
    </div>
    ...
    >
      <div>
        ...
        >
          ...
          <div
            class="w-32 h-14 bg-gray-800 bg-opacity-80 rounded-xl flex text-white"
          >
    ...
    <homePage v-if="this.hasGameUI" ref="homePage" />
    ...
    <playVideo
      v-if="sceneName == 'scene2'"
      ref="playVideo"
      video_url="http://router.aicodecloud.cn:8082/videos/111.mp4"
      type="video/mp4"
    />
...
export default {
  ...
  mounted() {
    ...
    if (this.$refs.playerSelectPanel && this.hasPlayerSelectPanel) {
      ...
      if (_Global.selectPlayerName) {
        avatarName = _Global.selectPlayerName;
      }
  ...
  methods: {
    ...
    ChangeScene(sceneSetting) {
      ...
      if (this.$refs.JoystickLeftPanel) {
        this.$refs.JoystickLeftPanel.SetDisplay(false);
      }
    },
    LeaveMeeting() {
    ...
    load3dComplete(callback) {
      ...
      if (this.$refs.JoystickLeftPanel) {
        this.$refs.JoystickLeftPanel.SetDisplay(true);
      }
